...
//        log.info("Authentication:"+auth2Authentication.toString());
        log.info("subscribeHeartBeat host");
        return Result.success("hello!");
    }

//    @SubscribeMapping({"/chat/info"})
//    public Result<String> subscribeChatInfo(@DestinationVariable String classId) {
//        log.info("subscribeState ");
//        return Result.success("班级消息推送订阅成功!");
//    }
    ...
    public Result<Room> subscribeCreateRoom(@DestinationVariable String type, OAuth2Authentication auth2Authentication) {
        ...
        if (auth2Authentication!=null){
            ...
            if (StringUtils.isNotBlank(hostId)){
                ...
                return chatService.createRoom(hostId,null,type);
            }else{
    ...
    public Result<Room> requestJoinRoom(@DestinationVariable String roomId, OAuth2Authentication auth2Authentication) {
        ...
        if (auth2Authentication!=null){
            ...
            if (StringUtils.isNotBlank(hostId)){
                ...
                Result<Room> result = chatService.addMemberIntoRoom(roomId,hostId);
                if (result.getCode()==ResultCode.SUCCESS.getCode()){
                    chatService.sendRoomEvent(result.getData().getRoomId(),Event.buildBroadCast(EventEnum.EVENT_CHAT_JOIN,result.getData().getRoomId(),result.getData()));
    ...
    public Result<Room> requestLeaveRoom(@DestinationVariable String roomId, OAuth2Authentication auth2Authentication) {
        ...
        if (auth2Authentication!=null){
            ...
            if (StringUtils.isNotBlank(hostId)){
                ...
                Result<Room> result = chatService.deleteMemberFromRoom(roomId,hostId);
                if (result.getCode()==ResultCode.SUCCESS.getCode()){
                    chatService.sendRoomEvent(result.getData().getRoomId(),Event.buildBroadCast(EventEnum.EVENT_CHAT_LEAVE,result.getData().getRoomId(),result.getData()));
                }
                return result;
            }else{
                return Result.failure(ResultCode.AUTH_NEED_LOGIN,"未登录");
    ...
    public Result<String> ackRoom(@DestinationVariable String roomId, Ack ack) {
        ...
        if (StringUtils.isNotBlank(hostId)){
            return  chatService.updateAck(roomId,hostId,DateUtils.getCurrentDateTime());
        }else{
            return Result.failure(ResultCode.AUTH_NEED_LOGIN,"未登录");
        }
    ...
    public Result<String> sendRoomMessage(@DestinationVariable String roomId,Message message, OAuth2Authentication auth2Authentication) {
...
//            String hostId = SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString();
            if (StringUtils.isNotBlank(hostId)){
                ...
                return chatService.sendRoomMessage(roomId,message);
            }else{
                return Result.failure(ResultCode.AUTH_NEED_LOGIN,"未登录");
            }
        }else{
    ...
    public Result<String> ackPersonal(@DestinationVariable String userId, Ack ack) {
        ...
        String hostId = SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString();
        if (StringUtils.isNotBlank(hostId)){
            return chatService.updateAck(userId,hostId,DateUtils.getCurrentDateTime());
        }else{
            return Result.failure(ResultCode.AUTH_NEED_LOGIN,"未登录");
        }
...
//            String hostId = SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString();
            if (StringUtils.isNotBlank(hostId)&&StringUtils.isNotBlank(userId)){
                ...
                // Todo:要确定对方是否存在
                message.setGmtCreate(DateUtils.getCurrentDateTime());
                return chatService.sendPersonalMessage(userId,hostId,message);
            }else{
                return Result.failure(ResultCode.AUTH_NEED_LOGIN,"未登录");
            }
        }else{
